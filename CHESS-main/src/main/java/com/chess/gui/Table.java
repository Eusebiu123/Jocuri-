...
public final class Table extends Observable {
    ...
    private Color lightTileColor = Color.decode("#381c2a");
    private Color darkTileColor = Color.decode("#3c3b5f");
    ...
    private class TilePanel extends JPanel {
            ...
            if (Table.get().getHighlightLegalMoves()) {
                for (final Move move : pieceLegalMoves(board)) {
                    if (move.getDestinationCoordinate() == this.tileId) {
                        try {
                            URL resource = Main.class.getResource("/art/misc/green_dot.png");
                            add(new JLabel(new ImageIcon(ImageIO.read(new File(resource.toURI())))));
                        }
        ...
        private void assignTilePieceIcon(final Board board) {
            ...
            if(board.getPiece(this.tileId) != null) {
                try{
                    final BufferedImage image = ImageIO.read(Main.class.getResource(pieceIconPath +
